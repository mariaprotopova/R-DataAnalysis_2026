...
data <- readxl::read_excel('Beer_Dataset.xlsx')
data$...1 <- NULL

data_scaled<- as.data.frame(scale(data[, 2:5]))

ggplot(data_scaled, aes(Cost, Sodium)) +
  geom_point()

fig <- plot_ly(data_scaled, x = ~Calories, y = ~Sodium, z = ~Cost)
fig <- fig %>% add_markers()
...
d <- dist(data_scaled %>% select(Calories, Sodium, Cost, Alcohol), method = 'manhattan')
...
data_scaled$cluster <- as.factor(cutree(hc.ward, 3))
